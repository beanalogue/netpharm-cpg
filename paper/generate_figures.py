...
def make_figure3():
    ...
    # From Table S2 (corrected formula pool: Longdan Xiegan Tang / Zhenggan Xifeng
    # Tang replaced with Xuefu Zhuyu Tang / Buzhong Yiqi Tang per the real NIKOM CPG)
    htn_auroc = [0.655, 0.655, 0.659, 0.600, 0.655, 0.655, 0.640, 0.623, 0.644, 0.600]
    htn_sigp  = [0.0005, 0.0005, 0.0005, 0.030, 0.0005, 0.0005, 0.006, 0.014, 0.003, 0.046]
    ins_auroc = [0.596, 0.596, 0.587, 0.599, 0.596, 0.596, 0.607, 0.636, 0.622, 0.608]
    ins_sigp  = [0.054, 0.054, 0.081, 0.059, 0.054, 0.054, 0.048, 0.027, 0.026, 0.041]
    dem_auroc = [0.571, 0.571, 0.571, 0.571, 0.571, 0.571, 0.557, 0.574, 0.557, 0.557]
    dem_sigp  = [0.146, 0.146, 0.146, 0.146, 0.146, 0.146, 0.188, 0.136, 0.188, 0.188]
    ...
    # Fill for significant region (above Bonferroni for HTN)
    # Shade scenarios where HTN is significant
    ...
    # Mark duplicate scenarios (2, 5, 6) with open markers (identical to primary)
    for idx in [1, 4, 5]:   # 0-based: scenarios 2, 5, and 6
    ...
    # Mark significant HTN scenarios with * below the marker
    ...
    ax.text(10.35, 0.502, "chance\n(0.5)", va="bottom", ha="left",
            fontsize=6.5, color="#888888", clip_on=False)
    ax.text(10.35, 0.656, "primary\n(0.655)", va="bottom", ha="left",
            fontsize=6.5, color=C_SIG, alpha=0.7, clip_on=False)

    # OB-cached annotation
    ax.annotate(
        "‡ OB cached\nat 30%",
        xy=(5.5, 0.542), xytext=(5.2, 0.519),
        fontsize=6.5, color="#888888",
        arrowprops=dict(arrowstyle="-", color="#BBBBBB", lw=0.8),
        ha="center",
    )

    # OT page-cap annotation (scenario 2 duplicates primary for same reason
    # OpenTargets page is already exhausted at OT>=0.10 for all three diseases)
    ax.annotate(
        "† OT page-\ncapped",
        xy=(2, 0.596), xytext=(1.55, 0.519),
        fontsize=6.5, color="#888888",
        arrowprops=dict(arrowstyle="-", color="#BBBBBB", lw=0.8),
        ha="center",
    )
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(labels_x, fontsize=6.8, ha="center")
    ax.set_ylabel("Pool AUROC", fontsize=9)
    ...
    # Shaded columns for the two scenarios where HTN is non-significant
    # (4 and 10: both OT >= 0.30, reduced disease gene set)
    for idx in [3, 9]:   # scenarios 4 and 10 (0-based)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color="none",  marker="o", ms=6, mfc="white",
               mec="#666666", mew=1.2, label="Duplicate of primary analysis (not independent)"),
    ...
    ax.set_title(
        "Figure 3. Sensitivity analysis — pool AUROC across 10 scenarios (7 independent)",
        fontsize=8, loc="left", pad=8, color=C_TEXT,
    )
