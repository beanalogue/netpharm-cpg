...
- A reverse network pharmacology pipeline was developed to evaluate whether CPG-recommended formula herbs show stronger disease-gene target overlap than non-recommended herbs across the full TCMSP herb space
...
- Differential performance across diseases is consistent with differential database coverage of each disease-herb pharmacological interface
- Systematic herb exclusions introduce conservative bias; the pipeline is applicable to Chinese, Korean, and Kampo CPGs
...
**Conclusions:** The differential pipeline performance is most parsimoniously interpreted as reflecting differential database coverage of the herb–disease pharmacological interface rather than differential clinical efficacy of the formulas, though these explanations cannot be distinguished from the present data alone. The reverse NP framework is applicable to Chinese medicine, Korean medicine, and Kampo CPGs, providing a cross-comparable molecular evidence layer for TM clinical guideline development. The rank-based Mann-Whitney enrichment approach is assumption-free, reproducible, and directly applicable to multi-herb formula pool evaluation.
...
To address this gap, we propose a reverse NP approach in which the analytical direction is reversed: starting from a disease, identifying its molecular target space, and asking whether CPG-recommended formula herbs show stronger disease-gene target overlap than non-recommended herbs across the full TCMSP herb space. All indexed herbs are ranked by their hypergeometric overlap with the disease gene set; the enrichment of CPG formula herbs at the top of this ranking is then quantified by the area under the receiver operating characteristic curve (AUROC), mathematically equivalent to the Mann-Whitney U statistic, with significance assessed by permutation testing. This test differs structurally from Gene Set Enrichment Analysis (GSEA) [17] — in GSEA, gene ranking by phenotype correlation and the gene set under test are independent, whereas here herb ranking and the formula pool share the same disease gene set — but addresses a specific, well-defined biological question: do CPG-recommended herbs systematically achieve better disease-gene target overlap than non-recommended herbs? The Mann-Whitney framework answers this question without distributional assumptions. AUROC summarises pipeline performance per disease, enables cross-disease comparison, and supports sensitivity analysis across pipeline parameters. We are not aware of a prior study that has applied a systematic computational framework to this comparative question in TM CPG evaluation. Systematic database exclusions — herbs absent from TCMSP's indexed set or failing pharmacokinetic filters — are documented with their directional effect, providing a transparent record of pipeline limitations.
...
Korean TM CPG recommendations (2021 editions, NIKOM) [6–8] were used as the primary ethnopharmacological ground truth. Three indications were selected by purposive sampling according to the following pre-specified criteria: (1) a published Korean TM CPG (NIKOM 2021 edition) with explicit evidence-graded herbal formula recommendations is available; (2) the indications span distinct pathophysiological domains — cardiometabolic (essential hypertension), neuropsychiatric (insomnia disorder), and neurodegenerative (dementia/cognitive impairment) — providing biological diversity for an initial demonstration of the pipeline; and (3) each indication represents a major disease burden in Korean clinical practice where TM is in established use. As a demonstration study, representativeness across all Korean TM CPG indications is not claimed; generalisability of the findings requires validation across a broader disease set and multiple CPG systems. Korean TM CPGs specify formula names and evidence grades (on an A–D GRADE-based scale) but not individual herb compositions. Standard herb compositions were sourced from the Chinese Pharmacopoeia 2020 (ChP 2020) [5], which serves as the authoritative reference for canonical formula composition in both Korean and Chinese TM practice.

All CPG-recommended formulas (evidence grades A–C; grades B and C only in the present dataset, as no A-grade herbal formula recommendations were identified across the three indications) for each disease were identified, and a union herb pool was constructed comprising all unique herbs across recommended formulas. Herbs absent from TCMSP's database or failing OB/DL filters were documented as systematic exclusions (Supplementary Table S1).

### 2.5 AUROC computation and permutation testing

The primary enrichment test asks whether CPG-recommended formula herbs rank significantly higher in disease-gene target overlap than non-formula herbs across the full TCMSP herb space. All 500 TCMSP herbs are first scored by their hypergeometric overlap with the disease gene set and ranked accordingly (Section 2.3); the formula pool is then evaluated by comparing the rank distribution of formula herbs against non-formula herbs using the area under the receiver operating characteristic curve (AUROC), equivalent to the Mann-Whitney U statistic. Because herb ranking is derived from the same disease gene set used to define formula pool membership (via CPG indication), this design differs from Gene Set Enrichment Analysis (GSEA) [17], where the gene ranking and the gene set are independent. The test nonetheless addresses a well-defined biological question — do CPG-recommended herbs achieve better disease-gene target overlap than non-CPG herbs? — and is answered by the Mann-Whitney framework without distributional assumptions. Significance is assessed by permutation of herb labels rather than by the Wilcoxon asymptotic approximation, which is unreliable for the small and highly unequal group sizes present in formula pool evaluations.
...
Minor discrepancies between Wilcoxon and permutation p-values (most apparent for insomnia: 0.069 vs 0.054) reflect the permutation test's assumption-free treatment of the small and unequal group sizes; the permutation p-value was pre-specified as the primary significance criterion. Per-formula AUROCs were computed as exploratory descriptive statistics. Post-hoc multiple testing correction (Bonferroni and Benjamini-Hochberg FDR) across all per-formula tests yielded zero significant results within any disease, consistent with the exploratory designation for these analyses.
...
An eleventh scenario evaluated robustness to TCMSP compound-target prediction confidence by restricting to targets with SVM score ≥ 0.7 or RF score ≥ 0.7 (high-confidence predictions). This scenario directly addresses the concern that unfiltered TCMSP predictions introduce noise into herb rankings through inclusion of low-confidence interactions.
...
OpenTargets queries (score ≥ 0.10) returned 500 targets for essential hypertension (MONDO_0001134), 500 for insomnia disorder (MONDO_0013600), and 499 for dementia/cognitive impairment (MONDO_0001627). For hypertension and insomnia, the 500-target count reflects the page-size cap of the OpenTargets GraphQL query rather than the true size of the association set, so the disease gene sets for these two indications are conservatively truncated. Dementia returned 499 targets — one below the page limit — indicating that the dementia gene set at OT ≥ 0.10 is complete and not truncated; consequently, the null pool AUROC for dementia does not arise from incomplete disease-side gene set representation. Of the 500 TCMSP herbs, 465 herbs (disease-independent) retained at least one active compound passing the OB ≥ 30%/DL ≥ 0.18 filter. After scoring against each disease's gene set, herbs with ≥ 1 shared target — and therefore included in the final ranked list — numbered 450 for hypertension, 412 for insomnia, and 456 for dementia.
...
| Disease | Formulas (n) | Pool herbs | Ranked | Excluded | Pool AUROC | 95% CI † | Wilcoxon p | Permutation p | Significant |
|---------|-------------|-----------|--------|---------|-----------|---------|-----------|--------------|------------|
| Essential hypertension | 5 | 32 | 27 | 5 | **0.655** | (0.543, 0.767) | 0.0035 | **< 0.001** | **Yes** |
| Insomnia disorder | 5 | 24 | 21 | 3 | 0.596 | (0.469, 0.723) | 0.069 | 0.054 | No |
| Dementia | 5 | 23 | 20 | 3 | 0.571 | (0.441, 0.701) | 0.142 | 0.146 | No |

*† 95% CI computed by normal approximation using Mann-Whitney standard error (SE = √[n₁n₂(n₁+n₂+1)/12] / (n₁n₂)); n₁ = ranked formula herbs, n₂ = ranked non-formula herbs. Bootstrap resampling (n = 10,000 iterations) produced concordant CIs (within ±0.01 of normal approximation boundaries), confirming the adequacy of the normal approximation at these sample sizes.*
...
### 3.5 Cross-disease specificity control

To assess whether the primary enrichment signals reflect disease-specific molecular alignment or general pharmacological richness of CPG formula herbs, a 3×3 cross-disease AUROC matrix was computed: each formula pool (HTN, INS, DEM) was ranked against each disease gene set (HTN, INS, DEM), yielding nine combinations (Supplementary Figure S1).

**Supplementary Table S3.** Cross-disease pool AUROC matrix (permutation p-values in parentheses).

| Formula herb pool | vs HTN gene set | vs INS gene set | vs DEM gene set |
|-------------------|----------------|----------------|----------------|
| HTN formula herbs | **0.642** (p=0.004) | 0.579 (p=0.073) | 0.593 (p=0.037) |
| INS formula herbs | 0.593 (p=0.066) | 0.555 (p=0.209) | 0.536 (p=0.295) |
| DEM formula herbs | 0.660 (p=0.006) | 0.623 (p=0.033) | 0.541 (p=0.275) |

*Bold = matched disease (formula indication = gene set disease). Diagonal cells are the cross-disease-controlled equivalents of the primary analysis. Permutation test, n = 1,000, seed = 42.*

The matrix reveals two distinct patterns. First, HTN formula herbs showed their strongest enrichment against the HTN gene set (AUROC = 0.642, p = 0.004) and non-significant enrichment against the INS gene set (AUROC = 0.579, p = 0.073), supporting partial disease-specific signal for the hypertension indication. The INS formula pool provides the most informative cross-disease test: INS herbs vs HTN gene set (AUROC = 0.593, p = 0.074 n.s.) is lower than the matched HTN herbs vs HTN (AUROC = 0.642, p = 0.004), confirming that the HTN enrichment signal is not reproduced by a clinically distinct formula pool. Second, INS formula herbs showed no significant enrichment against any gene set — consistent with the primary null-trend result — confirming that the INS formula pool does not carry a general pharmacological enrichment artefact. Third, DEM formula herbs showed higher enrichment against the HTN gene set (AUROC = 0.660, p = 0.006) than against their own DEM gene set (AUROC = 0.541, p = 0.275). This cross-disease enrichment is a structural consequence of formula pool overlap: 17 of 20 ranked DEM formula herbs (85%) are also present in the HTN formula pool, primarily because Buzhong Yiqi Tang (補中益氣湯) appears in both CPGs and the Dihuang-series formulas (Liuwei Dihuang Tang, Bawei Dihuang Wan) share most herbs with Qiju Dihuang Wan from the HTN pool. The 3 DEM-exclusive herbs absent from the HTN pool — Guizhi (*Cinnamomum cassia*, cinnamon twig), Fuzi (*Aconitum carmichaelii*, aconite), and Huanglian (*Coptis chinensis*) — each carry independently documented cardiovascular pharmacology (RAAS-modulating terpenoids, adrenergic alkaloids, and berberine's established antihypertensive mechanism respectively), which accounts for their non-trivial HTN gene set overlap. The DEM gene set's resistance to enrichment by its own formula herbs — despite being the only complete (non-truncated) gene set — reinforces the interpretation that the DEM null result arises from sparse compound-target annotation on the DEM formula side rather than from an absent molecular signal.

As a partial independent validation of the HTN enrichment signal, the pipeline was re-run using a genetics-only subset of the OpenTargets disease gene set, restricting to genes with positive genetic association evidence scores (GWAS, rare variant studies; OT datatype `genetic_association`, score ≥ 0.05; n = 872 genes, 66% overlapping with the primary 500-gene set). Against this partially independent gene set, the HTN formula herb pool achieved AUROC = 0.585 (permutation p = 0.069) — directionally consistent with the primary result but sub-threshold after Bonferroni correction. The attenuation reflects the partial independence of the two gene sets: TCMSP compound-target annotations are enriched for pharmacologically characterised druggable targets (NOS3, PTGS2, AGTR1) that score highly on drug and clinical evidence in OpenTargets but are less represented among GWAS-identified loci (FGF5, FURIN, SVEP1). The directional consistency supports genuine pharmacological alignment of the HTN formula pool, while the attenuation illustrates the gap between the druggable target layer captured by TCMSP and the broader genetic architecture of essential hypertension.
...
Scenario 11 (TCMSP target confidence filter: SVM score ≥ 0.7 or RF score ≥ 0.7) assessed robustness to prediction noise. Restricting to high-confidence interactions reduced the number of ranked herbs modestly (HTN: 465 → 461; INS: 427 → 384; DEM: 471 → 463). Essential hypertension enrichment remained significant and was numerically stronger (AUROC = 0.662; permutation p = 0.003), insomnia showed a positive but sub-threshold trend (AUROC = 0.618; permutation p = 0.039), and dementia remained null (AUROC = 0.506; permutation p = 0.451). The strengthening of the HTN result under stricter confidence filtering indicates that the enrichment signal is concentrated in high-confidence compound-target predictions and is not an artefact of low-confidence noise in the TCMSP prediction set.

---

## 4. Discussion

### 4.1 Differential database coverage as a framework for interpreting reverse NP performance

The three-disease gradient — significant, non-significant trend, and null — is consistent with differential database coverage: the extent to which current compound–target databases capture the disease-specific pharmacology of TM formula herbs in TCMSP's annotation space. We term this differential 'molecular legibility' and treat it as a falsifiable hypothesis: the number of approved drug classes acting on a disease's pathogenic pathways — a database-independent proxy for how well the disease-herb target interface is pharmacologically characterised — should predict pipeline AUROC. Essential hypertension has more than ten distinct approved drug classes targeting RAAS, adrenergic, calcium channel, and diuretic mechanisms, representing the most densely annotated disease-target space among the three indications. Insomnia disorder has three to four drug classes (benzodiazepines, Z-drugs, orexin antagonists, melatonin agonists) with partially overlapping molecular targets. Dementia has two approved symptomatic drug classes (acetylcholinesterase inhibitors and NMDA receptor antagonist memantine) with limited target pathway specificity relative to the breadth of its proposed mechanisms. This ordering — established from regulatory records independently of any database query — matches the observed AUROC gradient (HTN > INS > DEM), consistent with the molecular legibility hypothesis and providing a database-independent plausibility check on the differential coverage interpretation. Diseases treated primarily through mechanisms not yet captured in compound–target databases are predicted to yield null results regardless of clinical efficacy of the formulas. This interpretation is more parsimonious than differential clinical efficacy, but these two explanations cannot be distinguished on the basis of the present data alone — a distinction with important implications for how computational NP results should be interpreted in the context of traditional medicine evaluation.

For essential hypertension, database coverage of the herb–disease target interface is high. The renin-angiotensin-aldosterone system, NOS3-mediated vascular tone, and sympathetic adrenergic pathways are well-characterized in both OpenTargets and TCMSP: many hypertension formula herbs contain active compounds with documented NOS3, ACE, PTGS2, and ADRB1 interactions. The two highest-ranked formula herbs, Duzhong (杜仲, *Eucommia ulmoides*) and Gouteng (鉤藤, *Uncaria rhynchophylla*), are key herbs in CPG-recommended hypertension formulas [7] and achieved high NP ranks through target overlap with RAAS and adrenergic pathway genes — precisely the molecular pathways most densely annotated for essential hypertension in OpenTargets. This convergence between the disease target space and herbal compound space in current databases accounts for the significant pool AUROC of 0.655. Per-herb cross-disease ranking comparison provides direct evidence that this enrichment reflects disease-specific molecular alignment rather than general pharmacological richness: Duzhong (杜仲, *Eucommia ulmoides*) and Gouteng (鉤藤, *Uncaria rhynchophylla*) — the two highest-ranked HTN formula herbs (ranks 12 and 19 against the HTN gene set) — ranked substantially lower against the insomnia gene set (ranks 30 and 40) and dementia gene set (ranks 46 and 41), indicating that their high NP scores are driven by HTN-relevant target profiles rather than target-set size alone. In contrast, broadly active herbs present across multiple formula pools — Huangqi (*Astragalus membranaceus*; HTN rank 92, INS rank 97) and Renshen (*Panax ginseng*; HTN rank 74, INS rank 48) — showed comparably moderate rankings across all gene sets, consistent with broad pharmacological profiles that do not preferentially align with any single disease. This within-herb pattern complements the cross-disease pool analysis (Section 3.5): the hypertension enrichment signal is anchored by herbs with demonstrably HTN-preferential molecular profiles, while the elevated cross-disease enrichment of the DEM formula pool against the HTN gene set is attributable to broadly active herbs shared between the HTN and DEM pools via Buzhong Yiqi Tang. Notably, the AUROC remains above 0.600 even when the OpenTargets score threshold is raised to 0.30, though the permutation p-value (0.030) falls below the Bonferroni-corrected threshold (α = 0.0167), indicating that restricting to only high-confidence gene associations attenuates but does not abolish the enrichment signal. The 5 HTN-exclusive herbs absent from both the DEM and INS formula pools — Duzhong, Gouqizi, Juhua, Sangjisheng, and Yimucao — together with other HTN-only herbs yield an AUROC of 0.637 (permutation p = 0.043) when evaluated as a subset of the formula pool, providing additional evidence that the enrichment signal is anchored in herbs clinically distinctive to the HTN indication rather than in herbs shared across multiple CPG indications.

For insomnia, database coverage of the herb–disease target interface is intermediate. The relevant neurobiology — serotonergic, GABAergic, histaminergic, and adenosine pathways — is annotated in OpenTargets, but with less target specificity than hypertension. Critically, several pharmacologically active insomnia formula herbs contain sedative compounds that fail OB/DL filters owing to their hydrophilic nature; saponins in Suanzaoren (酸棗仁), for example, require gut microbiota-mediated deglycosylation before absorption and are therefore not captured by standard oral bioavailability estimation models. This creates a systematic gap between the compounds that mediate clinical efficacy and those represented in the pipeline.

For dementia, database coverage of the herb–disease target interface is currently insufficient; this finding should not be interpreted as evidence of pharmacological inactivity. Notably, unlike hypertension and insomnia — where the OpenTargets gene set is conservatively truncated at the 500-row page limit — the dementia gene set is complete at the primary threshold (499 targets, below the page cap). The null result therefore does not arise from incomplete disease-side representation; rather, it is localized to the compound side: TCMSP target annotations for the key dementia formula herbs (Huanglian, Zhizi, Shichangpu) do not resolve against OpenTargets dementia-associated genes at the primary threshold. The cross-disease control analysis (Section 3.5) confirms this interpretation: DEM formula herbs yield null enrichment against DEM genes (AUROC = 0.541, p = 0.275) even as they yield significant enrichment against the HTN gene set (AUROC = 0.660, p = 0.006) — a pattern that is primarily a structural consequence of formula pool overlap (17 of 20 DEM-ranked herbs are shared with the HTN pool; Section 3.5) rather than a pharmacological richness effect: the 3 DEM-exclusive herbs actually have smaller target sets than the HTN-exclusive herbs (mean |T| = 37 vs 118), and the high cross-disease AUROC reflects the shared herb core rather than broad target profiles specific to the DEM pool. Yigan San (抑肝散, Yokukansan) carries meta-analytic RCT evidence for behavioural and psychological symptoms of dementia [10], and several dementia formulas in the pool are traditionally indicated for cognitive and neurological decline, with plausible mechanistic rationale in the TM literature. The null pool AUROC (0.571; permutation p = 0.146) is more consistent with the interpretation that the mechanisms of these formulas — anti-neuroinflammation, synaptic protection, cholinergic modulation — are insufficiently represented in current TCMSP compound-target annotations for the herbs involved, particularly Huanglian, Zhizi, and Shichangpu.
...
A related concern is research attention bias: TCMSP compound-target annotations are proportional to research investment, and CPG-recommended herbs — by virtue of being widely prescribed — attract more pharmacological study than obscure herbs, potentially inflating their target set size and hypergeometric rank independently of true disease-specific pharmacology. Several observations argue against this explaining the hypertension result. First, dementia formula herbs are equally or more extensively studied in the traditional medicine literature, yet yield null enrichment against dementia genes; if attention bias were the primary driver, enrichment would be expected for dementia as well. Second, Scenario 11 — which restricts to high-confidence (SVM/RF ≥ 0.7) compound-target predictions, where annotation inflation from poorly validated interactions is reduced — produces a stronger HTN enrichment (AUROC = 0.662, p = 0.003), not a weaker one, indicating the signal is concentrated in high-confidence annotations rather than in the long tail of speculative predictions. Third, the per-herb cross-disease ranking pattern — HTN-specific herbs ranking high against HTN genes but substantially lower against INS and DEM gene sets — is inconsistent with a general annotation-density artefact, which would inflate rankings equally across all gene sets. Attention bias cannot be fully excluded as a contributing factor, and its systematic quantification — for example by testing whether TCMSP target set size predicts herb rank independently of disease gene overlap — is identified as a methodological priority for future work.
...
A fundamental principle of multi-herb TM prescriptions is that therapeutic efficacy may emerge from the union target coverage of the formula as a whole rather than from the additive contributions of individual constituent herbs — a combinatorial logic encoded in the classical 君臣佐使 (principal–associate–adjuvant–guide) compositional framework. The present pipeline ranks individual herbs independently, so a formula in which no single herb achieves high disease-target overlap on its own — but whose constituent herbs collectively tile the relevant target space — would receive a low per-herb ranking despite genuine formula-level disease relevance. This represents a systematic conservative bias: the observed pool AUROC measures the enrichment of individual CPG formula herbs in the ranked herb list, not the enrichment of CPG formula-level target unions. It is therefore a lower bound on what a formula-resolution enrichment analysis would yield. The practical implication is that formulas such as Yigan San — which showed near-random per-herb AUROC (0.500) despite meta-analytic RCT support for BPSD [10] — may achieve their clinical effects through collective target coverage that is invisible to the single-herb ranking approach. Future analyses implementing set-level enrichment, in which each formula is represented by its union target set and ranked against alternative herb combinations of equivalent size, would provide a methodologically complete evaluation of formula-disease alignment and could substantially increase detection power for formulas that operate through distributive multi-target mechanisms.
...
Fourth, this study demonstrates the pipeline across three diseases. Generalizing the observed performance gradient requires validation across a larger disease set and multiple TM CPG systems.
...
Sixth, the pipeline ranks individual herbs independently and therefore does not capture formula-level combinatorial target coverage — the union of targets across constituent herbs of a single formula may align with disease biology even when no individual herb achieves high overlap. This is a conservative bias: enrichment at the herb level is a lower bound on enrichment at the formula level. Set-level enrichment analysis evaluating each formula's union target set is identified as a priority methodological extension.

Seventh, high formula pool overlap across different disease indications limits the discriminative power of cross-disease specificity controls. In this study, 85% of DEM formula herbs (17/20) and 74% of INS formula herbs (17/23) are also present in the HTN formula pool — a structural consequence of widely recommended multi-indication formulas (Buzhong Yiqi Tang, Dihuang-series, Xuefu Zhuyu Tang). This overlap means the cross-disease AUROC matrix cannot cleanly isolate disease-specific enrichment for the DEM pool. The INS pool comparison (AUROC = 0.593, p = 0.074 n.s. against the HTN gene set) remains more informative due to 6 INS-exclusive herbs attenuating the cross-disease score. Future cross-disease controls in multi-indication TM CPG studies should stratify the analysis by pool-exclusive vs shared herbs to separate disease-specific signal from the shared herbal core.

Eighth, independent cross-database validation of the HTN enrichment signal faces access constraints: DisGeNET cross-validation was not possible due to paid API restrictions, and GWAS Catalog-derived associations were underpowered (n = 26 genome-wide significant loci; n = 37 at p < 10⁻⁵), too few for stable hypergeometric scoring within the TCMSP compound–target space. A partial internal validation using only the genetics-only subset of OpenTargets (GWAS + rare variant evidence; n = 872 genes, score ≥ 0.05) produced AUROC = 0.585 (permutation p = 0.069) — directionally consistent with the primary result but sub-threshold after Bonferroni correction (Section 4.3). The attenuation relative to the primary result reflects the partial coverage of GWAS-derived loci by pharmacokinetic-filtered TCMSP compound-target annotations. Validation using DisGeNET or a purpose-built genetic association database with adequate locus coverage remains a priority for future work.

Ninth, the three indications were selected by purposive sampling and do not constitute a representative sample of all Korean TM CPG indications. The observed three-disease performance gradient — significant, non-significant trend, and null — should not be interpreted as a general property of Korean TM CPGs as a whole. Validation across a larger set of indications and CPG systems is required before broader conclusions about the relationship between TM clinical practice and molecular pharmacology can be drawn.

---

## 5. Conclusions

We developed a reverse network pharmacology pipeline that evaluates whether CPG-recommended formula herbs show stronger disease-gene target overlap than non-recommended herbs across the full TCMSP herb space, and applied it to three Korean TM CPG indications. The pipeline identified significant enrichment of CPG formula herbs for essential hypertension (pool AUROC = 0.655; permutation p < 0.001, Bonferroni-corrected), a non-significant positive trend for insomnia (AUROC = 0.596; p = 0.054), and null results for dementia (AUROC = 0.571; p = 0.146). Sensitivity analysis confirmed significant hypertension enrichment in 5 of 7 independently-tested parameter configurations; insomnia and dementia results were non-significant throughout. Systematic herb exclusions introduce conservative bias throughout, implying that the significant hypertension result is a lower bound on the true molecular enrichment.

The differential performance is most parsimoniously interpreted as reflecting differential database coverage of the herb–disease pharmacological interface — a hypothesis we term molecular legibility — rather than differential clinical efficacy of the formulas, though these explanations cannot be distinguished from the present data. The findings highlight specific, addressable gaps in current resources: phenolic glycoside compounds are systematically excluded by standard OB/DL filters despite documented in vivo activity, and pharmacologically important herbs such as Tianma remain outside TCMSP's indexed set. Resolving these gaps would directly improve pipeline sensitivity for diseases where TM formulas act through glycoside-mediated or poorly annotated mechanisms. The framework is generalizable to Chinese medicine, Korean medicine, and Kampo CPGs, offering a standardised molecular evidence layer that could support more informed and computationally transparent TM clinical guideline development.
