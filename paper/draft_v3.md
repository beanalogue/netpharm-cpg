...
The matrix reveals two distinct patterns. First, HTN formula herbs showed their strongest enrichment against the HTN gene set (AUROC = 0.642, p = 0.004) and non-significant enrichment against the INS gene set (AUROC = 0.579, p = 0.073), supporting partial disease-specific signal for the hypertension indication. Second, INS formula herbs showed no significant enrichment against any gene set — consistent with the primary null-trend result — confirming that the INS formula pool does not carry a general pharmacological enrichment artefact. Third, DEM formula herbs showed higher enrichment against the HTN gene set (AUROC = 0.660, p = 0.006) than against their own DEM gene set (AUROC = 0.541, p = 0.275). This cross-disease enrichment is attributable to two sources: (a) overlapping herb composition between the HTN and DEM formula pools — Buzhong Yiqi Tang (補中益氣湯) appears in both, contributing Huangqi, Renshen, and Baizhu, which carry broad target profiles — and (b) a pharmacological richness effect whereby herbs with large compound-target sets in TCMSP accumulate overlap with the HTN gene set, which is the most densely annotated of the three in OpenTargets. The DEM gene set's resistance to enrichment by its own formula herbs — despite being the only complete (non-truncated) gene set — reinforces the interpretation that the DEM null result arises from sparse compound-target annotation on the DEM formula side rather than from an absent molecular signal.
...
Seventh, although cross-disease controls (Section 3.5) support partial disease specificity for the HTN enrichment, they also reveal a pharmacological richness effect: formula herbs with large compound-target sets in TCMSP tend to accumulate overlap with any densely annotated gene set, particularly HTN, regardless of their clinical indication. The pipeline does not currently decouple disease-specific target overlap from general pharmacological richness; future analyses could address this by regressing AUROC on herb target-set size or by normalising rankings within target-size-matched strata.

Eighth, independent cross-database validation of the HTN enrichment signal was attempted using GWAS Catalog-derived essential hypertension associations as a substitute gene set, but proved underpowered: only 26 genome-wide significant loci (37 at p < 10⁻⁵) were available, which is insufficient for stable hypergeometric scoring within the TCMSP compound–target space and produced unreliable AUROC estimates. DisGeNET cross-validation was not possible due to API access restrictions. Disease specificity is currently supported by the cross-disease AUROC control (Section 3.5) and per-herb ranking comparison (Section 4.1); validation against an independent, adequately powered gene set remains an open priority.
