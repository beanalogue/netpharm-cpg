...
*† 95% CI computed by normal approximation using Mann-Whitney standard error (SE = √[n₁n₂(n₁+n₂+1)/12] / (n₁n₂)); n₁ = ranked formula herbs, n₂ = ranked non-formula herbs.*
...
As a partial independent validation of the HTN result, we re-ran the pipeline using a genetics-only subset of the OpenTargets disease gene set — restricting to genes with positive genetic association evidence scores (GWAS, rare variant studies; OT datatype `genetic_association`, score ≥ 0.05), which is conceptually distinct from the compound-target annotations in TCMSP. This genetics-only gene set contained 872 genes (66% overlap with the primary 500-gene set; 372 additional genes absent from the primary set). Against this independent gene set, the HTN formula herb pool achieved AUROC = 0.585, permutation p = 0.069 — directionally consistent with the primary result (AUROC = 0.655) but sub-threshold after Bonferroni correction. The attenuation is interpretable: TCMSP compound-target annotations disproportionately capture pharmacologically characterised molecular targets (NOS3, PTGS2, AGTR1) that score highly on drug/clinical evidence in OpenTargets; GWAS loci (FGF5, FURIN, SVEP1) represent partially distinct biology not captured by standard pharmacokinetic-filtered compound libraries. The directional consistency supports that the HTN enrichment reflects genuine pharmacological alignment rather than circular artefact, while the attenuation at genetic-only evidence highlights the gap between TCMSP's pharmacological target space and the broader genetic architecture of hypertension.
